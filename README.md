...
| **LCDwiki 4.0" ESP32-32E** (E32R40T) | `320x480` **ST7796S** panel over 4-wire SPI on a **classic ESP32-WROOM-32E** (`4MB` flash, **no PSRAM**), with **XPT2046** resistive touch and an onboard RGB LED. Same lcdwiki ESP32-32E family as the 2.8" CYD clone but a bigger panel. Use the `e32r40t` firmware build. Due to RAM limits, this board supports **1 printer only**. The buzzer is disabled by default (audio is a DAC + amp the GPIO backend cannot drive). The touch chip shares the LCD's SPI bus, so it runs through LovyanGFX's shared-bus touch path - **the touch is unverified on hardware**; report if taps do not register. Pinout from the [LCDwiki wiki](https://www.lcdwiki.com/4.0inch_ESP32-32E_Display); see issue [#175](https://github.com/Keralots/BambuHelper/issues/175). | _community_ |
...
- **Multi-printer support** - monitor 2 printers simultaneously on ESP32-S3 boards; PSRAM-equipped boards add an experimental opt-in for up to 4 (see Multi-Printer Monitoring for the list); CYD, TZT, E32R40T, and ESP32-C3 have an experimental opt-in 2-printer mode but default to 1 printer
