...
| | **Waveshare ESP32-S3-Touch-LCD-2.8C** | `480x480` **round** **ST7701** IPS panel on the ESP32-S3's LCD_CAM RGB bus (16-bit parallel, with a 3-wire SPI command interface), ESP32-S3R8 (`16MB` flash / `8MB` PSRAM), and **GT911** capacitive touch. Use the `ws_lcd_28c` firmware build. Gets its own `480x480` round layout - the 240x240 round dashboard at exactly 2x, with a larger font tier - and supports **up to 2 printers** (up to 4 with the experimental opt-in). LCD reset, LCD CS, touch reset and the buzzer all hang off an on-board **TCA9554** I2C IO expander; the buzzer is therefore on/off only, so melodies play as rhythm without pitch. Battery voltage is read on GPIO4. IMU, RTC and microSD are not wired up in firmware. **Not hardware-tested** - the RGB pixel clock starts conservative at 12 MHz and is an `.ini` knob; see issue [#171](https://github.com/Keralots/BambuHelper/issues/171). Product page: [waveshare.com/esp32-s3-touch-lcd-2.8c.htm](https://www.waveshare.com/esp32-s3-touch-lcd-2.8c.htm) | _community_ |
